...
def run_tests(module, to_test, tests_root_dir, test_data, dir_names, default_timeout, abbr_len):
    ...
    while True:
        ...
        if len(stderr) > 0:
            ...
            outp = out.rstrip('\n')[:abbr_len] + (out.rstrip('\n')[abbr_len:] and '... (string abbreviated, full string in the output directory)')
            stdoutp = stdout.rstrip('\n')[:abbr_len] + (stdout.rstrip('\n')[abbr_len:] and '... (string abbreviated, full string in the results directory)') 
            if color == None:
